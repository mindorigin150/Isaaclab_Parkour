...
class ParkourManagerBasedRLEnv(ParkourManagerBasedEnv, gym.Env):
    ...
    def step_no_reset(
        self, action: torch.Tensor, active_mask: torch.Tensor | None = None
    ) -> VecEnvStepReturn:
        """Advance one vector step without resetting terminated slots."""
        return self._step(action, reset_done=False, active_mask=active_mask)
    ...
    def _step(
        ...
        *,
        ...
        self._latency_eval_active_mask = active_mask
        try:
            # process actions
            ...
            # -- check terminations
            previous_term_dones = {
                name: self.termination_manager.get_term(name).clone()
